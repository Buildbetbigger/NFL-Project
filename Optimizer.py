...
Version: 2.0.2 - FIXED: Genetic algorithm now respects user-configured salary cap
...
class FileHandler:
    ...
    def _estimate_ownership(salaries: np.ndarray, projections: np.ndarray, 
        ...
        position_multipliers = {
            'QB': 1.3, 'WR': 1.2, 'RB': 1.1, 'TE': 0.9,
            'K': 0.7, 'DST': 0.8, 'D': 0.8, 'FLEX': 1.0
        }
    ...
    def create_sample_csv() -> pd.DataFrame:
        """Create sample CSV in DFF format"""
        return pd.DataFrame({
            ...
            with st.spinner("Loading and validating data..."):
                ...
                if df is not None:
                    ...
                    if "auto-estimated" in message.lower():
                        with st.expander("About Auto-Estimated Ownership"):
                            st.markdown("""
...
def render_game_info():
    ...
    st.header("Step 2: Configure Game")
    ...
    try:
        ...
        with st.expander("Salary Analysis", expanded=True):
            ...
            if max_possible < salary_cap:
                st.warning(f"Even using the most expensive players, max spend is {UIHelpers.format_currency(max_possible)} (under the {UIHelpers.format_currency(salary_cap)} cap)")
        ...
        if config['use_genetic']:
            ...
            # FIXED: Pass salary_cap parameter to genetic algorithm
            ga_optimizer = GeneticAlgorithmOptimizer(df, game_info, mc_engine, salary_cap=salary_cap)
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if total_salary > salary_cap:
                        st.warning(f"Lineup exceeded cap ({UIHelpers.format_currency(total_salary)} > {UIHelpers.format_currency(salary_cap)}), skipping...")
                        continue
            ...
            if len(lineups) < target_lineups:
                if len(lineups) > 0:
                    st.warning(f"Generated {len(lineups)}/{target_lineups} lineups. Consider lowering diversity or min salary.")
                else:
                    st.error(f"""
...
def main():
    ...
    <div style='text-align: center; color: gray; padding: 20px;'>
        <p>NFL DFS AI-Driven Optimizer v2.0.2 | Built with AI</p>
        <p>For entertainment purposes only. Always gamble responsibly.</p>
    </div>
    """, unsafe_allow_html=True)
