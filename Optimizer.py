    ...
    class ShowdownOptimizer:
        ...
        def _validate_and_prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
            ...
            # AUTO-FIX SALARY FORMAT
            min_sal = df['Salary'].min()
            max_sal = df['Salary'].max()
    
            # Detect and fix salary format issues
            if max_sal <= 20:
                # Salaries in thousands format (5.5 = $5,500)
                df['Salary'] = (df['Salary'] * 1000).astype(int)
                self.logger.log(f"Auto-converted salaries from thousands (x1000)", "INFO")
                min_sal = df['Salary'].min()
                max_sal = df['Salary'].max()
            elif max_sal > 50000:
                # Salaries too high, try dividing
                df['Salary'] = (df['Salary'] / 100).astype(int)
                self.logger.log(f"Auto-converted salaries by dividing by 100", "WARNING")
                min_sal = df['Salary'].min()
                max_sal = df['Salary'].max()
    
        # Log salary range
        self.logger.log(f"Salary range: ${min_sal:,} - ${max_sal:,}", "INFO")
    
        # Validate reasonable DK range (with tolerance)
        if min_sal < 200 or max_sal > 15000:
            self.logger.log(
                f"WARNING: Unusual salary range: ${min_sal:,} - ${max_sal:,}",
                "WARNING"
            )
            # Only error if REALLY outside bounds
            if min_sal < 100 or max_sal > 20000:
                raise ValueError(
                    f"Salary values outside valid DraftKings range. "
                    f"Found: ${min_sal:,} - ${max_sal:,}. "
