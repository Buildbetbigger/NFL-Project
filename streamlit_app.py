    ...
    def run_optimization():
        ...
        try:
            ...
            if config['use_genetic']:
                ...
                for r in results:
                    captain_sal = df.loc[df['Player'] == r['captain'], 'Salary'].iloc[0] if not df[
                        df['Player'] == r['captain']].empty else 0
                    flex_sal = sum(
                        df.loc[df['Player'] == p, 'Salary'].iloc[0] for p in r['flex'] if not df[df['Player'] == p].empty)
                    total_sal = captain_sal * OptimizerConfig.CAPTAIN_MULTIPLIER + flex_sal
    
                    captain_proj = df.loc[df['Player'] == r['captain'], 'Projected_Points'].iloc[0] if not df[
                        df['Player'] == r['captain']].empty else 0
                    flex_proj = sum(df.loc[df['Player'] == p, 'Projected_Points'].iloc[0] for p in r['flex'] if
                                    not df[df['Player'] == p].empty)
                ...
                max_possible_salary = df['Salary'].max() * OptimizerConfig.CAPTAIN_MULTIPLIER + df['Salary'].nlargest(
                    5).sum()
                ...
                for attempt in range(max_attempts):
                    try:
                        ...
                        # CONSTRAINT 6: Uniqueness - exclude previously used lineups
                        for used_lineup in used_lineups:
                            ...
                            # This lineup can have at most 5 of the 6 players from any previous lineup
                            prob += (
                ...
                if len(lineups) < target_lineups:
                    st.warning(
                        f"⚠️ Generated {len(lineups)} lineups (requested {target_lineups}). Try relaxing constraints or increasing player pool.")
