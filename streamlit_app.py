...
def run_optimization():
    ...
    try:
        ...
        if config['use_genetic']:
            ...
            lineups_generated = 0
            used_lineups = set()
            max_attempts = config['num_lineups'] * 5
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    # Variables
                    captain_vars = {p: LpVariable(f"capt_{i}", cat=LpBinary) for i, p in enumerate(df['Player'])}
                    flex_vars = {p: LpVariable(f"flex_{i}", cat=LpBinary) for i, p in enumerate(df['Player'])}
                    ...
                    # Salary with flexible cap
                    salary_cap = df['Salary'].max() * OptimizerConfig.CAPTAIN_MULTIPLIER + df['Salary'].nlargest(5).sum()
                    salary_cap = min(salary_cap, OptimizerConfig.SALARY_CAP)
                    ...
                    if LpStatus[prob.status] == 'Optimal':
                        captain = [p for p in df['Player'] if captain_vars[p].varValue == 1]
                        ...
                        if lineup_key not in used_lineups:
                            ...
                            prob += lpSum([captain_vars[captain]]) + lpSum([flex_vars[p] for p in flex]) <= 5
